...
export function PriceParsePanel() {
  ...
  const parse = async () => {
    if (!text.trim()) { toast.error("Pehle supplier ka quote/email paste karein"); return; }
    setBusy(true);
    try {
      ...
      if (!rows.length) { toast.error("Text me koi jaana-pehchana material nahi mila"); return; }
  ...
  const accept = async (p: any) => {
    ...
    if (!confirm(`${mat?.name} ka rate ${formatINR(p.current_rate ?? 0)} → ${formatINR(p.proposed_rate)} karein?`)) return;
    try {
      ...
      toast.success("Material ka rate badal gaya");
  ...
  return (
    <Card>
      <CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2"><Receipt className="h-4 w-4" /> Supplier rate padho (AI)</CardTitle></CardHeader>
      <CardContent className="space-y-3">
        <p className="text-[11px] text-muted-foreground">Supplier ka quote/email paste karein. AI har material ka naya rate sujhaata hai; aapke accept karne par hi (record ke saath) material rate badlega. AI khud chup-chaap rate nahi badalta.</p>
        <Textarea rows={3} value={text} placeholder="Supplier ka quotation / email paste karein…" onChange={(e) => setText(e.target.value)} />
        <Button size="sm" onClick={parse} disabled={busy}>{busy ? <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" /> : <Receipt className="h-3.5 w-3.5 mr-1" />}Rate padho</Button>
            ...
            {propQ.data.map((p) => (
              <div key={p.id} className="text-xs border border-border rounded-md px-2 py-1.5 flex items-center justify-between gap-2">
                ...
                <span className="flex gap-1">
                  <Button size="sm" variant="outline" className="h-7" onClick={() => accept(p)}>Manzoor</Button>
                  <Button size="sm" variant="ghost" className="h-7" onClick={() => reject(p)}>Mana</Button>
                </span>
              </div>
            ))}
          </div>
